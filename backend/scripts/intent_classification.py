...
# Load Intent Labels
intent_labels = config['DEFAULT'].get('intent_classification_list')
...
def intent_classification(transcript):
    ...
    try:
        ...
        intent_labels_with_score = get_labels_with_score(result)
        end = time.time()
        print(f"Time Taken For Intent Classification: {end - start:.2f} seconds.")
        return intent_labels_with_score
    except Exception as error:
        print("Error in intent classification file: ", str(error))